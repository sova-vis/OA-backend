...
// ------------------------
async function fetchPaperFiles(offset, limit) {
    ...
    if (!needsJoin) {
        ...
        return paperFiles.filter((r) => (FILTER_FILE_TYPE ? r.file_type === FILTER_FILE_TYPE : true));
    }
    // Enrich paper_ids
    const paperIds = [...new Set(paperFiles.map((r) => r.paper_id).filter(Boolean))];
    ...
    const subjectIds = [...new Set((papers || []).map((p) => p.subject_id).filter(Boolean))];
    ...
    const filtered = paperFiles.filter((pf) => {
        if (FILTER_FILE_TYPE && pf.file_type !== FILTER_FILE_TYPE)
            return false;
        const p = paperMap.get(pf.paper_id);
        if (!p)
            return false;
        if (FILTER_YEAR && String(p.year) !== String(FILTER_YEAR))
            return false;
        const s = subjMap.get(p.subject_id);
        if (!s)
            return false;
        if (FILTER_LEVEL && String(s.level) !== String(FILTER_LEVEL))
            return false;
        if (FILTER_SUBJECT_CODE && String(s.code) !== String(FILTER_SUBJECT_CODE))
            return false;
        return true;
    });
    return filtered;
...
// Ingest one PDF
// ------------------------
async function ingestOne(pf) {
    const id = pf.id;
    const storagePath = pf.storage_path;
    ...
    const existingSet = new Set((existingEmbeds || []).map((r) => r.chunk_id));
    const toEmbed = dbChunks.filter((c) => !existingSet.has(c.id));
    console.log(`Need embeddings: ${toEmbed.length}`);
    // embed with small concurrency
    let embedsMade = 0;
    await runPool(toEmbed, EMBED_CONCURRENCY, async (c) => {
        ...
        embedsMade++;
        if (embedsMade % 5 === 0)
            console.log(`Embedded ${embedsMade}/${toEmbed.length}...`);
    });
