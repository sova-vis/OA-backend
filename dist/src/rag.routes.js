...
// ============================================================================
async function ragRetrieval(question, filters) {
    try {
        // 1. Embed question with Ollama
        const embedUrl = (process.env.OLLAMA_URL || "http://localhost:11434").replace(/\/$/, "") +
            "/api/embeddings";
        ...
        const questionEmbedding = embedData.embedding;
        ...
        const { data: searchResults, error: searchErr } = await supabase_1.supabase.rpc("rag_search", searchParams);
        ...
        const filteredResults = (searchResults || []).filter((r) => r.similarity >= SIMILARITY_THRESHOLD);
        ...
        const rawScores = filteredResults.map((r) => r.similarity);
        // 5. Get full chunk metadata
        const chunkIds = filteredResults.map((r) => r.chunk_id);
        const { data: enrichedChunks, error: enrichErr } = await supabase_1.supabase
            .from("rag_chunks")
            .select(`
        id,
        chunk_index,
      ...
      `)
        ...
        const similarityMap = new Map(filteredResults.map((r) => [r.chunk_id, r.similarity]));
...
// ============================================================================
async function generateExamAnswer(question, ragResult) {
...
Format your response EXACTLY as JSON:
{
  ...
  "common_mistakes": ["Mistake 1", "Mistake 2"]
                ...
                {
                    role: "user",
                    content: `Question: ${question}\n\nExam paper context:\n${contextChunks.substring(0, 2000)}`,
                },
            ],
            model: GROQ_MODEL,
            max_tokens: 700,
        ...
        if (Array.isArray(parsed.marking_points)) {
            markingPoints = parsed.marking_points.map((item) => {
        ...
        const avgSimilarity = ragResult.rawSimilarityScores.length > 0
        ...
        const coveragePercentage = Math.min(100, (ragResult.groupedResults.length / TOP_K) * 100);
        // Build citations with subject names
        const citations = ragResult.groupedResults
            .flatMap((group) => group.chunks.map((chunk) => ({
...
router.post("/query", async (req, res) => {
    try {
        const { question, limit = 5, filters } = req.body;
        ...
        `);
            ...
            if (metadata?.fileType) {
                results = results.filter((paper) => paper.paper_files?.some((pf) => pf.file_type === metadata.fileType));
            }
            // Group results by subject/year/session
            const grouped = results.reduce((acc, paper) => {
        ...
        if (!ragResult.success) {
            ...
            const response = {
                type: "exam_question",
                answer: "I don't have enough information in my database to answer this question with confidence. Try asking about specific exam topics or request a particular past paper.",
                citations: [],
                confidence_score: 0,
        ...
        if (!examAnswer) {
            ...
            const fallbackCitations = ragResult.groupedResults
                .flatMap((group) => group.chunks.map((chunk) => ({
                subject: group.subject,
                subjectName: getSubjectName(group.subject),
                year: group.year,
            ...
            const response = {
                ...
                coverage_percentage: Math.min(100, (ragResult.groupedResults.length / TOP_K) * 100),
            };
            return res.json(response);
        }
